...
export default function EditProduct({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (product && product.id) {
        ...
        setVariants(product.variants?.map((v: Variant & { sku?: string; imageIndex?: number | null }) => ({
          name: v.name, price: v.price, stock: v.stock, sku: v.sku || '', imageIndex: v.imageIndex ?? null,
        })) || [])
        // Load multi-select states
        const catIds = product.categories?.map((c: { id: string }) => c.id) || [product.categoryId]
  ...
  const handleRecrop = async (index: number) => {
    ...
    try {
      // Use canvas to convert image to blob (avoids CORS fetch issues)
