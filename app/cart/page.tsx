    ...
    <main className="container page-stack">
      ...
      <div className="cart-layout">
        ...
        <div className="cart-summary">
          ...
          <div className="cart-summary-row">
            <span>Shipping</span>
            <span>{freeShipping ? 'FREE' : 'Calculated at next step'}</span>
          </div>
          <hr className="cart-summary-divider" />
          <div className="cart-summary-total">
            <span className="cart-summary-total-label">Total({totalCount})</span>
            <span>
              <span className="cart-summary-total-price">${subtotal.toFixed(2)}</span>
              <span className="cart-summary-currency">USD</span>
            </span>
          </div>
          <button className="cart-checkout-btn">CHECK OUT</button>
          {!freeShipping && (
            <div className="cart-free-shipping">
              Add <strong>${(65 - subtotal).toFixed(2)}</strong> more for free shipping!
            </div>
          )}
        </div>
      </div>
