...
## Auth currently implemented
- Email/password registration + login via Prisma-backed API routes.
- Email verification with 6-digit code (`/api/auth/resend-verification`, `/api/auth/verify-email`, `/verify-email`).
- Google login via Auth.js route (`/api/auth/[...nextauth]`) when `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` are configured.
